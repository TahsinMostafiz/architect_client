...
const MyReviewsCard = ({ myReview, handleDelete }) => {
  const { _id, customer, serviceName, ratings, photoUrl, message } = myReview;

  return (
    <div className=" flex flex-col w-full max-w-lg p-6 mx-auto divide-y rounded-md divide-gray-700 bg-gray-900 text-gray-100">
      {myReview.length === 0 ? (
        <>
          <h3 className="text-4xl text-orange-600">No reviews were added</h3>
        </>
      ) : (
        <>
